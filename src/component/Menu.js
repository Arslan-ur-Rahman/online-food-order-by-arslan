...
{ cart[0] && (<div style={{display:"flex"}}class="alert alert-success" role="alert">
  You have {cart.length} Items In Your Cart &nbsp; <span style={{fontWeight:"bolder"}}>  {this.state.total} Pakistani Rupees. (Without Taxes) </span> 
...
<article data-aos="fade-right" id="article-card" style={{padding:"0px 2px 4px 2px",marginBottom:"7px"}} >
       ...
       <div style={{flexGrow:"1",display:"flex",flexWrap:"wrap"}}> {v.Qty.map((qty,j)=>{return(<div style={{flexGrow:"1",paddingBottom:"10px"}}> &nbsp; {qty} ({v.prices[j]} PKR;) :<input type="number" id="quantity"  onChange={this.handlechange.bind(this,qty,i)} name="quantity" placeholder="0" min="0" max="5" id="tentacles" name="tentacles" />  </div>)})}   </div>
